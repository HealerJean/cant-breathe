...
public class VirtualTopicReceiver {
	public static void main(String[] args) throws Exception {
		...
		for(int i=0;i<3;i++){
			MyThreadToppic t = new MyThreadToppic(cf);
			t.start();
			try {
				Thread.sleep(1000);
...
class MyThreadToppic extends Thread{
	private ConnectionFactory cf = null;
	public MyThreadToppic(ConnectionFactory cf){
		this.cf = cf;
	}
	public void run(){
		try{
           ...
           // Destination destination = session.createQueue("Consumer.A.VirtualTopic.Orders");
			 ...
			 * 自定义的前缀 拦截器中配置
			 */
			Destination destination = session.createQueue("VirtualTopicConsumers.A.VirtualTopic.Orders");
